...
public class PersonTaskService {
	...
	private void savePersonTask(ClassDefinition personTaskClassDefinition, PersonTask personTask) {
		...
		personTaskClassInstance.getProperties().stream().filter(p -> p.getName().equals("Starting Date")).forEach(p -> p.setValues(Lists.asList(personTask.getTaskDateFrom(), new Object[0])));
		personTaskClassInstance.getProperties().stream().filter(p -> p.getName().equals("End Date")).forEach(p -> p.setValues(Lists.asList(personTask.getTaskDateTo(), new Object[0])));
