...
public class ClassInstanceToTaskInstanceMapper implements AbstractMapper<ClassInstance, TaskInstance> {
	...
	@Autowired PropertyInstanceToTaskFieldMapper propertyInstanceToTaskFieldMapper;
	...
	private PropertyInstance<Object> findProperty(String name, List<PropertyInstance<Object>> properties) {
		PropertyInstance<Object> property = properties.stream().filter(p -> p.getName().equals(name)).findAny().orElse(null);
		return property;
	}
...
//	}
	...
	private ArrayList<ArrayList<PropertyInstance<Object>>> sortPropertiesByLevel(List<PropertyInstance<Object>> propertyInstances) {
	...
	public ClassInstance toSource(TaskInstance target) {
		...
		PropertyInstance<Object> startDate = propertyInstanceToTaskFieldMapper.toSource(target.getRequired().getStartDate());
		startDate.setLevel(1);
		findAndReplaceProperty(startDate, classInstance);
		
		PropertyInstance<Object> endDate = propertyInstanceToTaskFieldMapper.toSource(target.getRequired().getEndDate());
		endDate.setLevel(1);
		findAndReplaceProperty(endDate, classInstance);
		
		PropertyInstance<Object> description = propertyInstanceToTaskFieldMapper.toSource(target.getRequired().getDescripiton());
