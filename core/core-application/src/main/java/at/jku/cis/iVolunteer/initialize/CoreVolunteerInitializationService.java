...
public class CoreVolunteerInitializationService {
	...
	protected void registerVolunteers() {
		this.coreUserService.getCoreUsersByRole(UserRole.VOLUNTEER).forEach(volunteer -> registerVolunteer(volunteer,
				volunteer.getSubscribedTenants().stream().map(st -> st.getTenantId()).collect(Collectors.toList())));
	}
