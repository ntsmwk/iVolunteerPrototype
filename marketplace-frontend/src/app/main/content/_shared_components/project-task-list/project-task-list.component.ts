  ...
  ngOnInit() {
    this.marketplaceService.findById(this.marketplaceId).toPromise().then((marketplace: Marketplace) => {
      if (this.availableOnly === true) {
        this.taskService.findAvailableByParticipantAndProject(marketplace, this.participantId, this.projectId).toPromise().then((tasks: Array<Task>) => this.tasks = tasks);
      } else if (this.engagedOnly === true) {
        this.taskService.findEngagedByParticipantAndProject(marketplace, this.participantId, this.projectId).toPromise().then((tasks: Array<Task>) => this.tasks = tasks);
      } else if (this.finishedOnly === true) {
        this.taskService.findFinishedByParticipantAndProject(marketplace, this.participantId, this.projectId).toPromise().then((tasks: Array<Task>) => this.tasks = tasks);
      } else {
