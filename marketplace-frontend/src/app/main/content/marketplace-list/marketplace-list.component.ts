...
export class FuseMarketplaceListComponent implements OnInit {
  ...
  displayedColumns = ['name', 'shortName', 'url', 'actions', 'delete'];
  ...
  deleteMarketplace(marketplace: Marketplace) {
    this.marketplaceService.delete(marketplace)
      .toPromise()
      .then(() => 
        this.marketplaceService.findAll()
        .toPromise()
        .then((marketplace: Marketplace[]) => this.dataSource.data = marketplace));
  }
}
