          ...
          if (cell.value == "add") {
            ...
            for (let c of outer.configurableClasses) {
              if (c.id == parent.id) {
                ...
                let i = 0;
                for (let p of outer.properties) {
                  if (p.id == "maxgluehtemperatur") {
                    outer.properties.splice(i, 1);
                  }
                  i++;
                }
    ...
    try {
      for (let c of this.configurableClasses) {
        ...
        if (cell.id == "technische_beschreibung") {
          var overlay = new mx.mxCellOverlay(new mx.mxImage("/icons/technical.png", 30, 30), "Overlay", mx.mxConstants.ALIGN_LEFT, mx.mxConstants.ALIGN_TOP);
          this.graph.addCellOverlay(cell, overlay);
        }

        if (cell.id == "logistischeBeschreibung") {
          var overlay = new mx.mxCellOverlay(new mx.mxImage("/icons/operational.png", 30, 30), "Overlay", mx.mxConstants.ALIGN_LEFT, mx.mxConstants.ALIGN_TOP);
          this.graph.addCellOverlay(cell, overlay);
        }

        if (cell.id == "preislicheBeschreibung") {
          var overlay = new mx.mxCellOverlay(new mx.mxImage("/icons/price.png", 30, 30), "Overlay", mx.mxConstants.ALIGN_LEFT, mx.mxConstants.ALIGN_TOP);
          this.graph.addCellOverlay(cell, overlay);
        }

        if (cell.id == "root") {
          cell.value = "Haubenofen";
