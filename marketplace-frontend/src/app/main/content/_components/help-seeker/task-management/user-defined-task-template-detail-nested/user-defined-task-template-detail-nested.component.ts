...
export class NestedUserDefinedTaskTemplateDetailComponent implements OnInit {
  ...
  ngOnInit() {
    Promise.all([
      ...
      this.route.params.subscribe((params) =>
        this.loadProperty(params["marketplaceId"], params["templateId"])
      );
    });
  }
      ...
      .then((marketplace: Marketplace) => {
        ...
        Promise.all([
          this.userDefinedTaskTemplateService
            .getTemplate(marketplace, templateId)
            .toPromise()
            .then((template: UserDefinedTaskTemplate) => {
  ...
  private setUpDataSourcesAndExpandedStates() {
    for (const t of this.template.templates) {
      const dataSource: MatTableDataSource<TemplateProperty<
        any
      >> = new MatTableDataSource<TemplateProperty<any>>();
  ...
  editDescription() {
    this.dialogFactory
      .editTemplateDescriptionDialog(this.template)
      .then((description: string) => {
        if (!isNullOrUndefined(description)) {
          this.userDefinedTaskTemplateService
            .updateRootTaskTemplate(
              this.marketplace,
              this.template.id,
              null,
              description
  ...
  editName() {
    this.dialogFactory
      .editTemplateNameDialog(this.template)
      .then((name: string) => {
  ...
  removeTemplate() {
    this.dialogFactory
      ...
      .then((cont: boolean) => {
        if (cont) {
          this.userDefinedTaskTemplateService
            .deleteRootTaskTemplate(this.marketplace, this.template.id)
            .toPromise()
    ...
    this.dialogFactory.newTaskTemplateDialog().then((result: string[]) => {
      this.userDefinedTaskTemplateService
        ...
        .then((createdTemplate: UserDefinedTaskTemplate) => {
          this.template.templates.push(
            createdTemplate.templates[createdTemplate.templates.length - 1]
          );
          const newDataSource = new MatTableDataSource<TemplateProperty<any>>();

          newDataSource.data =
            createdTemplate.templates[
              createdTemplate.templates.length - 1
            ].templateProperties;
  ...
  removeSubTemplate(
    subtemplate: UserDefinedTaskTemplate,
    subTemplateIndex: number
  ) {
    this.dialogFactory
      .confirmationDialog(
        "Remove Sub-Template " + subtemplate.name + "...",
        "Are you sure you want to delete the Sub-Template " +
          subtemplate.name +
  ...
  editSubTemplateName(
    ...
    this.dialogFactory
      .editTemplateNameDialog(subtemplate)
      .then((name: string) => {
        if (!isNullOrUndefined(name)) {
          this.userDefinedTaskTemplateService
            .updateNestedTaskTemplate(
              this.marketplace,
              this.template.id,
              subtemplate.id,
              name,
            ...
            .then((updatedTemplate: UserDefinedTaskTemplate) => {
              if (!isNullOrUndefined(updatedTemplate)) {
                this.template.templates[subTemplateIndex].name =
                  updatedTemplate.templates[subTemplateIndex].name;
              }
            });
        }
      });
  ...
  editSubTemplateDescription(
    subtemplate: UserDefinedTaskTemplate,
    subTemplateIndex: number
  ) {
    this.dialogFactory
      .editTemplateDescriptionDialog(subtemplate)
      .then((description: string) => {
        if (!isNullOrUndefined(description)) {
          this.userDefinedTaskTemplateService
            .updateNestedTaskTemplate(
            ...
            .then((updatedTemplate: UserDefinedTaskTemplate) => {
              if (!isNullOrUndefined(updatedTemplate)) {
                this.template.templates[subTemplateIndex].description =
                  updatedTemplate.templates[subTemplateIndex].description;
              }
            });
        }
      });
  ...
  changeTemplateOrderDialog() {
    this.dialogFactory
      .changeSubtemplatesOrderDialog(this.template)
      .then((data: SortDialogData) => {
        if (!isNullOrUndefined(data)) {
          ...
          this.userDefinedTaskTemplateService
            .updateTemplateOrderNested(
              this.marketplace,
              this.template.id,
              data.list
            )
            .toPromise()
            .then((ret: UserDefinedTaskTemplate) => {
              console.log(ret);
              this.refresh();
            });
        }
  ...
  editProperties(
    ...
    this.router.navigate(
      [
        `/main/task-templates/user/edit/${this.marketplace.id}/${this.template.id}/${subtemplate.id}`,
      ],
      { queryParams: { ref: "nested" } }
    );
  }
  ...
  changePropertyOrder(
    ...
    this.dialogFactory
      .changePropertyOrderDialog(subtemplate.templateProperties)
      .then((data: SortDialogData) => {
        if (!isNullOrUndefined(data)) {
          for (let i = 0; i < data.list.length; i++) {
            data.list[i].order = i;
          ...
          this.userDefinedTaskTemplateService
            ...
            .then((updatedTemplate: UserDefinedTaskTemplate) => {
              this.template = updatedTemplate;
              this.dataSources[subTemplateIndex].data =
                updatedTemplate.templates[subTemplateIndex].templateProperties;
              // this.refresh();
            });
        }
      });
  ...
  viewPropertyDetails(
    subtemplate: UserDefinedTaskTemplate,
    templateProperty: TemplateProperty<any>,
    subTemplateIndex: number
  ) {
    this.router.navigate(
      [
  ...
  removeProperty(
    ...
    this.userDefinedTaskTemplateService
      .removePropertiesFromNestedTemplate(
        this.marketplace,
        this.template.id,
        subtemplate.id,
        [templateProperty.id]
