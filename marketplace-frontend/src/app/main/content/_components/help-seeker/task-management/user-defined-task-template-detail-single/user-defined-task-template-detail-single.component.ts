...
export class SingleUserDefinedTaskTemplateDetailComponent implements OnInit {
  ...
  ngOnInit() {
    Promise.all([
      ...
      this.route.params.subscribe((params) =>
        this.loadPropertiesFromTemplate(
            ...
            this.propertyDefinitionService
              ...
              .then(() => {
                if (!isNullOrUndefined(this.template)) {
                  this.questions = this.questionService.getQuestionsFromProperties(
  ...
  loadAllPropertyDefinitions() {
    this.propertyDefinitionService
      .getAllPropertyDefinitons(this.marketplace, this.helpseeker.tenantId)
      .toPromise()
      .then((propertyDefinitions: PropertyDefinition<any>[]) => {
        this.allPropertiesList = propertyDefinitions;
      });
  ...
  navigateEditForm() {
    this.router.navigate(
      [
        `/main/task-templates/user/edit/${this.marketplace.id}/${this.template.id}`,
      ],
      { queryParams: { ref: "single" } }
    );
  }
  ...
  deleteTemplate() {
    this.dialogFactory
      .confirmationDialog(
        "Are you sure?",
        "Are you sure you want to delete this Template? This action cannot be reverted"
      )
      .then((cont: boolean) => {
        if (cont) {
          this.userDefinedTaskTemplateService
  ...
  changePropertyOrderDialog() {
    this.dialogFactory
      .changePropertyOrderDialog(this.template.templateProperties)
