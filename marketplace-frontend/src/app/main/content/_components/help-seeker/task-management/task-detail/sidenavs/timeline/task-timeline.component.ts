...
export class FuseTaskTimelineComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.taskHistoryChangedSubscription = this.messageService.subscribe(
      "taskHistoryChanged",
      this.loadTaskInteractions.bind(this)
    );
  }
  ...
  private loadTaskInteractions() {
    ...
    this.marketplaceService
      ...
      .then((marketplace: Marketplace) => {
        this.taskInteractionService
          .findByTask(marketplace, <Task>{ id: this.taskId })
          .toPromise()
  ...
  getTaskInteractionsByDay(dayAsString: string) {
    return isNullOrUndefined(this.date2Interactions)
      ? []
      : this.date2Interactions.get(dayAsString);
  }
}
