...
export class FuseTaskDetailComponent implements OnInit {
  ...
  ngOnInit() {
    Promise.all([
      ...
      this.route.params.subscribe((params) =>
        this.loadTask(params["marketplaceId"], params["taskId"])
      );
    });
  }
        ...
        this.taskService
          ...
          .then((task: Task) => {
            ...
            this.workflowService
              .getProcessId(marketplace, task.id)
              .toPromise()
              .then((workflowProcessId: string) => {
                ...
                this.workflowService
                  ...
                  .toPromise()
                  .then((workflowSteps: WorkflowStep[]) => {
                    this.workflowStepsDefault = workflowSteps.filter(
                      (step: WorkflowStep) =>
                        step.workflowStepType === "DEFAULT"
                    );
                    this.workflowStepsSpecial = workflowSteps.filter(
                      (step: WorkflowStep) =>
                        step.workflowStepType === "SPECIAL"
                    );
  ...
  private handleSpecialWorkflowSteps() {
    this.workflowStepsAssignment = this.workflowStepsSpecial.filter(
      (step: WorkflowStep) =>
        step.label.toLowerCase() === "assign" ||
        step.label.toLowerCase() === "unassign"
    );
  }
