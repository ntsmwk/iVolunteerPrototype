...
export class ClassConfiguratorComponent implements OnInit, AfterContentInit {
  ...
  addClassWithRelationship(
    ...
    addedClass.name =
      ...
      ")";
    addedClass.tenantId = this.globalInfo.currentTenants[0].id;
  ...
  private openOverlay(cell: MyMxCell, event: mxgraph.mxEventObject) {
    if (
      ...
      this.overlayContent.marketplace = this.globalInfo.currentMarketplaces[0];
      this.overlayContent.userInfo = this.globalInfo.userInfo;
  ...
  showInstanceForm() {
    ...
    this.router.navigate(
      [`main/instance-editor/${this.globalInfo.currentMarketplaces[0].id}`],
      {
        queryParams: {
