...
export class AddClassDefinitionGraphDialogComponent implements OnInit {
  constructor(
    ...
    public dialogData: AddClassDefinitionGraphDialogData,
  ...
  async ngOnInit() {
    ...
    this.graphData = <ClassConfigurationDTO>(
      await this.classConfigurationServce
        .getAllForClassConfigurationInOne(
          this.globalInfo.currentMarketplaces[0],
          this.dialogData.matchingEntityConfiguration.classConfigurationId
        )
        .toPromise()
    );
    this.loaded = true;
  ...
  private initGraph() {
    ...
    this.graph.getCursorForCell = (cell: MyMxCell) => {
      if (
        ...
        cell.getStyle() === CConstants.mxStyles.classDisabled
      ) {
        return "default";
      }
    ...
    graph.addMouseListener({
      ...
      mouseMove: function (evt, state) {
        if (this.pointerOnClass(state)) {
          ...
          if (
            addedEntities.findIndex((e) => e.classDefinition.id === cell.id) !==
            -1
          ) {
            return;
          }
          ...
          outer.graph.setCellStyle(CConstants.mxStyles.classHighlighted, [
            cell,
          ]);
        } else {
          const { addedEntities } = outer.dialogData;
          ...
          if (
            (!isNullOrUndefined(this.highlightedCell) &&
              addedEntities.findIndex(
                (e) => e.classDefinition.id === this.highlightedCell.id
              ) === -1) ||
            !isNullOrUndefined(this.deselectedCell)
          ) {
            outer.graph.setCellStyle(CConstants.mxStyles.classNormal, [
      ...
      mouseUp: function (evt, state) {
        ...
        const {
          entities,
        } = outer.dialogData.matchingEntityConfiguration.mappings;
        const cell = state.state.cell as MyMxCell;
        ...
        if (
          outer.dialogData.addedEntities.findIndex(
            (e) => e.classDefinition.id === cell.id
          ) === -1
        ) {
          ...
          outer.dialogData.addedEntities = outer.dialogData.addedEntities.filter(
            (e) => e.classDefinition.id !== cell.id
          );
      ...
      pointerOnClass: function (state) {
        return (
          ...
          !isNullOrUndefined(state.state) &&
          !isNullOrUndefined(state.state.cell) &&
          state.state.cell.cellType === MyMxCellType.CLASS
        );
      },
    });
  }
  ...
  private addClassDefinitionCell(classDefinition: ClassDefinition): MyMxCell {
    const mxStyle =
      this.dialogData.existingEntityPaths.findIndex((p) =>
        p.endsWith(classDefinition.id)
      ) !== -1
        ? CConstants.mxStyles.classDisabled
        : this.dialogData.addedEntities.findIndex(
            (e) => e.classDefinition.id === classDefinition.id
          ) !== -1
    ...
    if (
      cell.getStyle() === CConstants.mxStyles.classDisabled ||
      cell.getStyle() === CConstants.mxStyles.classHighlighted
    ) {
  ...
  private addCheckedOverlay(cell: MyMxCell) {
    const overlay = new mx.mxCellOverlay(
      new mx.mxImage(
        "/assets/icons/class_editor/check-solid_white.png",
        14,
        14
      ),
      "Overlay",
  ...
  private createRelationshipCellById(relationship: Relationship): MyMxCell {
    const source: MyMxCell = this.graph
      .getModel()
      .getCell(relationship.source) as MyMxCell;
    const target: MyMxCell = this.graph
      .getModel()
      .getCell(relationship.target) as MyMxCell;
