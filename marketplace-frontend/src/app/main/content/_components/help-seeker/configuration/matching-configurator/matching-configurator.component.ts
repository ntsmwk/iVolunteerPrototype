...
import {
  ...
  MatchingEntityType,
  MatchingEntityMappings,
  MatchingEntity,
  MatchingDataRequestDTO,
} from "app/main/content/_model/matching";
...
export class MatchingConfiguratorComponent implements OnInit, AfterContentInit {
  ...
  async loadClassesAndRelationships(
    ...
    this.matchingCollectorConfigurationService
      ...
      .toPromise()
      .then((data: MatchingDataRequestDTO) => {
        this.data = data;
  ...
  ngAfterContentInit() {
    ...
    this.graph.getEdgeValidationError = (
      edge: MyMxCell,
      source: MyMxCell,
      target: MyMxCell
    ) => {
    ...
    if (!mx.mxClient.isBrowserSupported()) {
      ...
      this.graph.addListener(
        mx.mxEvent.CLICK,
        (sender: any, evt: mxgraph.mxEventObject) => {
  ...
  private insertClassDefinitionsLeft() {
    const title = this.graph.insertVertex(
      ...
      "left_header",
      this.data.matchingConfiguration.leftClassConfigurationName,
      SPACE_X,
      HEADER_Y,
      HEADER_WIDTH,
      HEADER_HEIGHT,
    ...
    const cell = this.insertClassDefinitionsIntoGraph(
      ...
      new mx.mxGeometry(
        SPACE_X + CLASSDEFINTIION_OFFSET_X,
        y,
        CLASSDEFINITION_WIDTH,
    ...
    const addButton = this.graph.insertVertex(
      ...
      "+",
      SPACE_X +
        CLASSDEFINTIION_OFFSET_X +
        CLASSDEFINITION_WIDTH / 2 -
        BUTTON_WIDTH / 2,
      y,
      BUTTON_WIDTH,
      Button_HEIGHT,
  ...
  private insertClassDefinitionsRight() {
    ...
    const xClassDefinition =
      x - CLASSDEFINITION_WIDTH - CLASSDEFINTIION_OFFSET_X - SPACE_X;
    let y = HEADER_Y;

    const title = this.graph.insertVertex(
      this.graph.getDefaultParent(),
      "right_header",
      this.data.matchingConfiguration.rightClassConfigurationName,
      xHeader,
      y,
      HEADER_WIDTH,
      HEADER_HEIGHT,
    ...
    const cell = this.insertClassDefinitionsIntoGraph(
      this.data.matchingConfiguration.rightAddedClassDefinitionPaths,
      this.data.rightMappingConfigurations.mappings,
      new mx.mxGeometry(
        xClassDefinition,
        y,
        CLASSDEFINITION_WIDTH,
    ...
    const addButton = this.graph.insertVertex(
      ...
      "right_add",
      "+",
      xClassDefinition + CLASSDEFINITION_WIDTH / 2 - BUTTON_WIDTH / 2,
      y,
      BUTTON_WIDTH,
      Button_HEIGHT,
      CConstants.mxStyles.matchingAddButton
  ...
  insertClassDefinitionsIntoGraph(
    ids: string[],
    ...
    for (const id of ids) {
      ...
      geometry = new mx.mxGeometry(
        cell.geometry.x,
        cell.geometry.y + cell.geometry.height + CLASSDEFINTIION_SPACE_Y,
        cell.geometry.width,
  ...
  insertClassDefinition(
    id: string,
    matchingEntity: MatchingEntity,
    geometry: mxgraph.mxGeometry,
    pathPrefix: "r" | "l"
  ): MyMxCell {
    let cell = new mx.mxCell(
      matchingEntity.classDefinition.name,
      geometry,
    ...
    cell = this.addPropertiesToCell(
      cell,
      matchingEntity,
      PROPERTY_SPACE_X,
      CLASSDEFINITION_HEAD_HEIGHT + PROPERTY_SPACE_Y,
      pathPrefix
    );
    ...
    const numberOfProperties =
      isNullOrUndefined(matchingEntity.classDefinition.properties.length) ||
      matchingEntity.classDefinition.properties.length === 0
        ? 0
        : matchingEntity.classDefinition.properties.length;
    const rectHeight =
      numberOfProperties === 0
        ? CLASSDEFINITION_HEAD_HEIGHT
        : CLASSDEFINITION_HEAD_HEIGHT +
          PROPERTY_SPACE_Y +
          numberOfProperties * PROPERTY_HEIGHT +
          PROPERTY_SPACE_Y;
  ...
  private constructRemoveOverlay() {
    const overlay = new mx.mxCellOverlay(
      new mx.mxImage(
        "/assets/icons/class_editor/times-solid_white.png",
        14,
        20
      ),
      "Overlay",
  ...
  private insertMatchingOperatorsAndRelationships() {
    for (const entry of this.data.relationships) {
      ...
      if (!isNullOrUndefined(entry.leftMatchingEntityPath)) {
        const leftCellId =
          "l" + this.data.pathDelimiter + entry.leftMatchingEntityPath;
      ...
      if (!isNullOrUndefined(entry.rightMatchingEntityPath)) {
        const rightCellId =
          "r" + this.data.pathDelimiter + entry.rightMatchingEntityPath;
  ...
  private async performSave() {
    ...
    await this.matchingConfigurationService
      .saveMatchingConfiguration(
        this.marketplace,
        this.data.matchingConfiguration
      )
      .toPromise();
    await this.matchingOperatorRelationshipService
      .saveMatchingOperatorRelationships(
        this.marketplace,
        this.data.relationships,
        this.data.matchingConfiguration.id
      )
      .toPromise();
    this.redrawContent();
  }
  ...
  handleMousedownEvent(event: any, item: any, graph: mxgraph.mxGraph) {
    ...
    const removeEventListeners = () => {
      ...
      this.graphContainer.nativeElement.removeEventListener(
        "dragover",
        onDragOver
      );
    };
  ...
  private constructConnector(
    ...
    if (!isNullOrUndefined(sourceOffsetX)) {
      cell.geometry.setTerminalPoint(
        new mx.mxPoint(coords.x + sourceOffsetX, coords.y),
        true
      );
    }
    if (!isNullOrUndefined(targetOffsetX)) {
      cell.geometry.setTerminalPoint(
        new mx.mxPoint(coords.x + targetOffsetX, coords.y),
        false
      );
    }
  ...
  handleClickEvent(event: mxgraph.mxEventObject) {
    ...
    if (
      isNullOrUndefined(cell) ||
      isNullOrUndefined(cell.cellType) ||
      this.displayOverlay
    ) {
      return;
    }
    ...
    if (event.properties.event.button === 0) {
      if (cell.cellType === MyMxCellType.ADD_CLASS_BUTTON) {
        ...
        this.dialogFactory
          ...
          .then((ret: AddClassDefinitionDialogData) => {
            if (
              !isNullOrUndefined(ret) &&
              !isNullOrUndefined(ret.addedEntities)
            ) {
              if (cell.id === "left_add") {
                this.data.matchingConfiguration.leftAddedClassDefinitionPaths.push(
                  ...ret.addedEntities.map((e) => e.path)
                );
              } else {
                this.data.matchingConfiguration.rightAddedClassDefinitionPaths.push(
                  ...ret.addedEntities.map((e) => e.path)
                );
              }
              this.redrawContent();
  ...
  deleteOperators(cells: MyMxCell[]) {
    ...
    try {
      ...
      this.data.relationships = this.data.relationships.filter(
        (r) =>
          cellsToRemove.findIndex(
            (c) =>
              c.cellType === MyMxCellType.MATCHING_OPERATOR && r.id === c.id
          ) < 0
      );
    } finally {
      this.graph.getModel().endUpdate();
  ...
  deleteClassDefinitionCell(deleteCell: MyMxCell) {
    ...
    try {
      ...
      if (deleteCell.id.startsWith("l")) {
        this.data.matchingConfiguration.leftAddedClassDefinitionPaths = this.data.matchingConfiguration.leftAddedClassDefinitionPaths.filter(
          (path) => path !== actualId
        );
        const relationship = this.data.relationships.find((r) =>
          r.leftMatchingEntityPath.startsWith(actualId)
        );
        ...
        this.data.matchingConfiguration.rightAddedClassDefinitionPaths = this.data.matchingConfiguration.rightAddedClassDefinitionPaths.filter(
          (path) => path !== actualId
        );
        const relationship = this.data.relationships.find((r) =>
          r.rightMatchingEntityPath.startsWith(actualId)
        );
  ...
  handleOverlayOpened(event: mxgraph.mxEventObject, cell: MyMxCell) {
    this.overlayRelationship = this.data.relationships.find(
      (r) => r.id === cell.id
    );
