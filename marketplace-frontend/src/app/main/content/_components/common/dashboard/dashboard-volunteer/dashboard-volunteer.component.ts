...
export class DashboardVolunteerComponent implements OnInit {
    ...
    try {
      const localClassInstances = <ClassInstance[]>(
        await this.localRepositoryService
          .findClassInstancesByVolunteer(this.volunteerInfo)
          .toPromise()
      );
  ...
  async syncOneToLocalRepository(ciDTO: ClassInstanceDTO) {
    ...
    this.localRepositoryService
      .synchronizeSingleClassInstance(this.volunteerInfo, ci)
      .toPromise()
      .then(() => {
    ...
    if (
      ...
      this.dialogFactory
        ...
        .then((ret: boolean) => {
          if (ret) {
            this.localRepositoryService
              .removeSingleClassInstance(this.volunteerInfo, ciDTO.id)
              .toPromise()
              .then(() => {
  ...
  async syncAllToLocalRepository() {
    ...
    this.localRepositoryService
      .synchronizeClassInstances(this.volunteerInfo, missingCis)
      .toPromise()
      .then(() => {
  ...
  async shareClassInstanceToIssuer(ci: ClassInstanceDTO) {
    ...
    const share = <ClassInstance>(
      await this.localRepositoryService
        .getSingleClassInstance(this.volunteerInfo, ci.id)
        .toPromise()
    );
    const list = [share];
